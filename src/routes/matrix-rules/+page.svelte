<script lang="ts">
</script>

<div class="content-box">
	<h1>Matrix Rules</h1>

	<h3>#1 No Direct Selling</h3>
	<h5>Please don't sell anything directly here</h5>
    <br>

    <h3>#2 Be Nice!</h3>
    <h5>Be nice, respectful and supportive of each other. No racism, homophobia, bigotry, or hate. Everyone is welcome in this community. Not everyone has the same level of knowledge and there is no such thing as a stupid question. Please do not put others down for reaching out.</h5>
    <br>

    <h3>#3 No Bad Advice</h3>
    <h5>Do not spread wrong and/or harmful advice in bad faith</h5>
    <br>

    <h3>#4 No Doxxing</h3>
    <h5>Do not give out personal details or information about anyone without their explicit consent</h5>
    <br>

    <h3>#5 No Advertising</h3>
    <h5>Do not advertise your product, service, social media, whatever here</h5>
    <br>

    <h3>#6 No Spam</h3>
    <h5>Spamming for the sake of spamming is not allowed</h5>
    <br>

    <h3>#7 English Only</h3>
    <h5>We are a very international community, so we have to agree on a common language to use. This is English.</h5>
    <br>

    <h3>#8 The Unspoken Rules</h3>
    <h5>This ruleset won't ever be able to cover every situation in every aspect. Moderators may decide themselves whether you're doing something wrong.</h5>
</div>

<style>
	@import url('https://fonts.googleapis.com/css2?family=Concert+One&family=Poppins&display=swap');

	.content-box {
		background-color: #242424;
		border-radius: 25px;
		padding: 3rem;
		max-width: 75%;
		display: block;
		color: white;
		margin-bottom: 5rem;
		font-size: x-large;
		margin-left: 50%;
		transform: translateX(-50%);
        text-align: center;
        margin-top: 3rem;
	}

	.content-box h1 {
		font-family: 'Concert One', cursive;
		margin-bottom: 2rem;
	}

	@media screen and (max-width: 800px) {
		.content-box {
			width: 100%;
		}
	}
</style>
